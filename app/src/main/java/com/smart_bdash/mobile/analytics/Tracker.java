...
import com.smart_bdash.mobile.analytics.notification.BDashNotification;
...
public class Tracker {
     ...
     * SDK Tracker パラメーター
     **/
    protected String screenName;      // スクリーン名
    protected String loginUser;       // ユーザー名
    protected String relationalKey;   // 外部キー
    protected String relationalValue; // 外部バリュー
    protected String bootType;        // 起動タイプ
    protected String bootValue;       // 起動バリュー
    ...
    private void init(Context context, String uuid) {
        ...
        // adID はユーザー同意が得られている場合のみ取得する
        // 同意が未取得の場合は取得せず、同意設定後の次回初期化時に取得する
        if (MobileSDKManager.getInstance().hasUserConsent()) {
            fetchAdId();
        }
    }

    /**
     * 広告ID(adId)を取得して保存します<br>
     *  ・ユーザー同意が得られている場合のみ SDK 初期化時に呼び出されます
     */
        ...
        Thread localThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    User.getInstance().setCanUseAdId(true);
