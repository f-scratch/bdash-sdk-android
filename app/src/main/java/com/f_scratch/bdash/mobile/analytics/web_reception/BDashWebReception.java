...
//    private final static String SCHEMA_COMMAND  = "command://";
    ...
    private class WebViewCallback {
        ...
        @JavascriptInterface
        public void exist2call( final String func, String value ) {
            final boolean isExist = Boolean.valueOf(value);

            printConsole( "### func " + func + "() is exist: " + value );
            if( isExist ) {
                if( webView!=null ){
                    webView.post(new Runnable() {
                        ...
                        public void run() {
                            if( webView!=null ) {
                                try {
                                    webView.loadUrl("javascript:android." + func + "(" + func + "())");
                                }catch( Exception e ) {
                    ...
                    // WebView インスタンスがないってことはすでにクローズ処理が行われている
                }
        ...
        @JavascriptInterface
        public void exist2callWithFinish( final String func, String value ) {
            final boolean isExist = Boolean.valueOf(value);
                ...
                if( webView!=null ) {
                    webView.post(new Runnable() {
                        ...
                        public void run() {
                            if (webView != null) {
                                try {
                                    webView.loadUrl("javascript:android.dummy(" + func + "())");
                                } catch (Exception e) {
                                    // Null Reference で 実際にクラッシュ発生
                                }
                                onPageFinished();
        ...
        public void dummy( String value ){
            LogUtil.s( "### dummy result: " + value );
        }

        @JavascriptInterface
        public void onClose( String value ){
            LogUtil.s( "### onClose result: " + value );
            dismiss();
        }
        ...
        public void viewSource(final String src) {
            ...
            notifyWebReceptionListener(src);
        }

        // たぶん未使用
        @JavascriptInterface
        public void exist( String func, String value ) {
            printConsole( "### func " + func + "() is exist: " + value );
    ...
    private WebViewClient webViewClient = new WebViewClient(){
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            if (isEmbedded) {
                view.loadUrl("javascript:android.viewSource(document.getElementsByTagName('html')[0].outerHTML);");
            } else {
    ...
    void getStatusCodeByJavascript( WebView webView ) {
        ...
        // 関数の存在チェック
        webView.loadUrl("javascript:android.exist2callWithFinish('" + JS_FUNC_SUCCESS + "',  typeof " + JS_FUNC_SUCCESS + " == 'function')");
    }
    ...
    void closeByJavascript( WebView webView ) {
        ...
        webView.loadUrl("javascript:android.exist2call('" + JS_FUNC_CLOSE + "',  typeof " + JS_FUNC_CLOSE +" == 'function')");
    }
    ...
    private Runnable bootTrackingRunnable = new Runnable() {
        ...
        public void run() {
            ...
            // memo: タスク実行モニタリング. monitor で実行確認済
/*
            try {
                Thread.sleep(5000);
            }catch( Exception e ){
...
*/
    ...
    static void requestWebReception( BDashReport report, final BDashWebReception webReception){
        ...
        String request = null;
        try {
            request = com.f_scratch.bdash.mobile.analytics.util.LogicUtil.createJsonRequestWithCommonParameter(report);
        }catch( Exception e ) {
            e.printStackTrace();
        }
    ...
    void updateLayout(){
        ...
        try {
            ...
            ViewGroup rootView = (ViewGroup) getDialog().getWindow().getDecorView().findViewById(R.id.com_f_scratch_bdash_mobile_rootView);
            View closeButton = getCloseButtonView(rootView);
            WebView webView    = (WebView)rootView.findViewById(R.id.com_f_scratch_bdash_mobile_reception_webView);
            ViewGroup webViewFrame = (ViewGroup)rootView.findViewById(R.id.com_f_scratch_bdash_mobile_reception_webViewFrame);
            ...
            for( int i=0 ; i< webViewFrame.getChildCount() ; i++ ) {
                ...
                if( target instanceof WebView ) {
                    ...
                    LogUtil.s( postData );
                    printConsole( current.getUrl() );

                    if( current.forceShow==null ) {
                        ((WebView) target).postUrl(current.getUrl(), postData.getBytes());
                    }else{
    ...
    private void commandSchema( BDashSchema schema ){
        ...
        // コピー コマンド
            ...
            String message = String.format("%s %s", clip.getDescription().getLabel(), context.getResources().getString(R.string.com_f_scratch_bdash_mobile_command_copied));
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = activityInflater.inflate(R.layout.com_f_scratch_bdash_mobile_reception_fragment_webview, container, false);
        ...
        webView = view.findViewById(R.id.com_f_scratch_bdash_mobile_reception_webView);
        webView.getSettings().setJavaScriptEnabled(true);
        webView.setScrollBarStyle(View.SCROLLBARS_OUTSIDE_OVERLAY);
        webView.setWebViewClient(webViewClient);
        webView.getSettings().setMixedContentMode(WebSettings.MIXED_CONTENT_COMPATIBILITY_MODE);
        // Javascript 連携
        webView.addJavascriptInterface(new WebViewCallback(), "android");
        // Cookie の許諾
    ...
    private ImageView getCloseButtonView(View rootView) {
        ...
        if (isCloseButtonAdjustment()) {
            ...
            closeButton = rootView.findViewById(R.id.com_f_scratch_bdash_mobile_reception_closeButton_adjustable);
        } else {
            LogUtil.s(">>  close button can not adjustment.");
            closeButton = rootView.findViewById(R.id.com_f_scratch_bdash_mobile_reception_closeButton);
        }
        ...
        executor.execute(() -> {
            ...
            mainHandler.post(() -> {
                if (buttonSrc != null) {
                    ...
                    imageView.setImageResource(R.drawable.com_f_scratch_bdash_mobile_close_100x100);
                }
            });
        });
    }
